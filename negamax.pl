...
negamax(A,Suiv,Pmax,Pmax,[_,H]):-
	heuristique(A,Suiv,H).

negamax(A,Suiv,Pnew,Pmax, [C,VN]):-
	successeurs(A,Suiv,Succ),
	loop_negamax(A,Pnew,Pmax,Succ,Vsuiv),
	meilleur(Vsuiv,[C,V]),
	VN is 0-V.
...
meilleur([[C,V]],[C,V]).
meilleur([[C,V]|Reste], Meilleur_Couple):-
	Reste\=[],
	meilleur(Reste,[Cm,Vm]),
	(
		(Vm < V) ->
		Meilleur_Couple=[Cm,Vm];
		Meilleur_Couple=[C,V]
	).
